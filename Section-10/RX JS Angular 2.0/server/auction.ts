...
import {Host, Review, getHosts, getHostById, getReviewsByHostId} from './model';
...
app.get('/hosts', (req, res) => {
  res.json(getHosts(req.query));
});

app.get('/hosts/:id', (req, res) => {
  res.json(getHostById(parseInt(req.params.id)));
});

app.get('/hosts/:id/reviews', (req, res) => {
  res.json(getReviewsByHostId(parseInt(req.params.id)));
});
...
wsServer.on('connection', ws => {
  ws.on('message', message => {
    ...
    subscribeToHostBids(ws, subscriptionRequest.productId);
  });
});
...
function subscribeToHostBids(client, productId: number): void {
